...
void main() {
  ...
  group('Get Movie Detail', () {
    ...
    blocTest<MovieDetailBloc, MovieDetailState>(
      'Should emit proper states when fetching movie recommendations',
      build: () {
        when(
          mockGetMovieRecommendations.execute(tId),
        ).thenAnswer((_) async => Right(testMovieList));
    ...
    blocTest<MovieDetailBloc, MovieDetailState>(
      'Should emit error when fetching movie recommendations fails',
      build: () {
        when(
          mockGetMovieRecommendations.execute(tId),
        ).thenAnswer((_) async => const Left(ServerFailure('Server Failure')));
      ...
      act: (bloc) => bloc.add(const FetchMovieRecommendations(tId)),
      expect: () => [MovieDetailError('Server Failure')],
      verify: (_) {
        verify(mockGetMovieRecommendations.execute(tId));
      },
    );
    ...
    blocTest<MovieDetailBloc, MovieDetailState>(
      ...
      expect: () => [MovieDetailError('Database Failure')],
    );

    blocTest<MovieDetailBloc, MovieDetailState>(
      'Should emit error when removing from watchlist fails',
      build: () {
    ...
    blocTest<MovieDetailBloc, MovieDetailState>(
      'Should update watchlist status when movie detail is loaded',
      build: () {
    ...
    blocTest<MovieDetailBloc, MovieDetailState>(
      'Should emit loading when watchlist status is checked without movie detail',
      build: () {
        when(mockGetWatchlistStatus.execute(tId)).thenAnswer((_) async => true);
        return movieDetailBloc;
      },
